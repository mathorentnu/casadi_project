...
def solve(x_initial):
    ...
    for i in range(N + 1):
        ...
        cost_term = c_v * v_i ** exponent + c_delta * delta_i ** exponent

        dist_cost_upper = fmax(0, y_i - map_upper_limit + dist_limit) ** 2
        dist_cost_lower = fmax(0, dist_limit - (fmax(x_i, y_i) - map_limit)) ** 2
        ...
        if i == 0:
            ...
            f += cost_term
            f += c_v_diff * (v_i - v_prev) ** 2
            f += c_dist*dist_cost_upper
            f += c_dist_lower*dist_cost_lower
...
upper_corner = (-10, map_upper_limit)
...
if plot:
    ...
    fig, ax = plt.subplots()
    get_rect = patches.Rectangle(corner, width=width - extra_dilation,
                                 height=height - extra_dilation, color=color)
    upper_rect = patches.Rectangle(upper_corner, width=30, height=10, color=color)
    ...
    fix, ax = plt.subplots()
    ax.plot(v_final, label='v')
    ax.plot(delta_final, label='w')
    ax.legend()
    plt.show()
